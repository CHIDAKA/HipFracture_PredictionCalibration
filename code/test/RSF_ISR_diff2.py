...
## 50%除くのを治さないといけない。ALEを５万軒から計算するのも追加


# ratio = 1 argparseで指定
n_rep = 100
TARGET_N = 100000 #目的より大きいサイズ
True_TARGET_N = 50000
...
# data load
    ...
    filepath = f"./data/ratio_{ratio}/disease.csv"
    ...
    if not os.path.exists(f"./data/calib_eval_data_v2.pkl"):
        ...
        with open(f"./data/ratio_{ratio}/calib_eval_data_v2.pkl", "wb") as f:
            pickle.dump((X_calib, y_calib, X_eval, y_eval), f)
    else:
        with open(f"./data/ratio_{ratio}/calib_eval_data_v2.pkl", "rb") as f:
    ...
    for i in range(n_rep):
        ...
        with open(f"{output_dir}Model_{i}_x1.pkl", "wb") as f:
        ...
        for name in ["RSF","RSF_ISR","RSF_dykstra","RSF_ISR_dykstra"]:
            ...
            met_path = f"{output_dir}metrics_diff_x1.txt"
            ...
            with open(met_path, mode) as f:
                pd.DataFrame([{
                    ...
                    "cindex": cindex,
                    "IBS": IBSS,
                    "model_path": f"{output_dir}Model_{i}_x1.pkl",
